...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            dbname=os.environ.get("DB_NAME", "FRED_ETL"),
...
def create_slicing_charts(df):
 ...
 # --- ADD THE BAR CHART HERE ---
    df_avg_jobs['year'] = df_avg_jobs['date'].dt.year
    yearly_jobs = df_avg_jobs.groupby('year')['total_nonfarm'].mean().reset_index()
    fig_yearly = px.bar(
        yearly_jobs,
        x='year',
        y='total_nonfarm',
        labels={'year': 'Year', 'total_nonfarm': 'Avg Jobs Created'},
        title=f"Yearly Average Jobs Created ({year_range[0]}-{year_range[1]})"
    )
...
def create_dicing_charts(df):
    ...
    # --- Dynamic Key Events Section ---
    st.markdown("*Key Events in Selected Quarter and Year Range:*")

    # Example key events dictionary (expand as needed)
    key_events = {
        (2010, 'Q1'): "Slow recovery from the Great Recession.",
        (2020, 'Q2'): "COVID-19 pandemic causes historic job losses.",
        (2020, 'Q3'): "Partial recovery as lockdowns ease.",
        (2022, 'Q1'): "Strong job growth post-pandemic.",
        # Add more (year, quarter): "event" pairs as needed
    }

    # Collect events for the selected range and quarter
    events_to_show = []
    for y in range(year_range[0], year_range[1] + 1):
        event = key_events.get((y, quarter), None)
        if event:
            events_to_show.append(f"<b>{y} {quarter}:</b> {event}")

    if events_to_show:
        st.markdown(
            f"""
            <div style="background-color:#d4f7d4; padding:16px; border-radius:8px;">
                <span style="color:#111; font-weight:bold; font-size:1.08em;">
                    {'<br>'.join(events_to_show)}
                </span>
            </div>
            """,
            unsafe_allow_html=True
        ...
        st.markdown(
            ...
            <div style="background-color:#d4f7d4; padding:16px; border-radius:8px;">
                <span style="color:#111; font-weight:bold; font-size:1.08em;">
                    No key events recorded for this selection.
                </span>
            </div>
            """,
            unsafe_allow_html=True
...
def main():
    ...
    st.markdown("**Developed by Juluri Preethi**")
